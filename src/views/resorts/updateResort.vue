<template>
  <div class="updateResort" v-if="currentResort">
    <h2>update resort</h2>
    <form @submit.prevent="upload">
      ...
      <div class="thumbnail">
        ...
        <div class="thumbEl">
          <img :src="store.state.url + '/' + currentResort.thumbnail.path" ref="thumbEl" alt="" />
        </div>
      </div>
      <div class="btn">
        <button
          :class="['button', resortsModule.canUpdate ? '' : 'cantPost']"
          @click="[router.push('/hotels&resorts/resorts'), (shouldUpload = false)]"
        >
          cancel
        </button>
        <button v-html="store.state.resortsModule.updateButton" class="button"></button>
      </div>
    </form>
  </div>
</template>
...
const resorts = store.state.resortsModule.resorts
const resortsModule = store.state.resortsModule
const currentResort = resorts.find((r) => r._id == currentRoute.params.resort_id)
const stars = ref('')
const name = ref('')
const loc = ref('')
const desc = ref('')
...
if (currentResort) {
  loc.value = currentResort.location
  name.value = currentResort.name
  desc.value = currentResort.desc
}
...
const upload = () => {
  if (shouldUpload.value) {
    if (resortsModule.canUpdate) {
      ...
      store.commit('updateResort', data)
    }
  }
}
</script>
