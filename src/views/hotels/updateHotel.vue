<template>
  <div class="updateHotel" v-if="currentHotel">
    <h2>update hotel</h2>
    <form @submit.prevent="upload">
      ...
      <div class="thumbnail">
        ...
        <div class="thumbEl">
          <img :src="store.state.url + '/' + currentHotel.thumbnail.path" ref="thumbEl" alt="" />
        </div>
      </div>
      <div class="btn">
        <button
          class="button"
          @click="[(shouldUpload = false), $router.push('/hotels&resorts/hotels')]"
        >
          cancel
        </button>
        <button
          v-html="store.state.hotelsModule.updateButton"
          :class="['button', hotelsModule.canUpdate ? '' : 'cantPost']"
        ></button>
      </div>
    </form>
  </div>
...
const hotels = store.state.hotelsModule.hotels
const currentHotel = hotels.find((h) => h._id == currentRoute.params.hotel_id)
const stars = ref('')
const name = ref('')
const loc = ref('')
const desc = ref('desc')
...
if (currentHotel) {
  loc.value = currentHotel.location
  name.value = currentHotel.name
  desc.value = currentHotel.desc
}
