<template>
  <div class="postVisa">
    ...
    <form @submit.prevent="update">
        ...
        <div class="input country">
          ...
          <div class="select">
            <div class="countriesForm" ref="countiresFormEl">
              <div class="current" @click="countiresForm = !countiresForm" v-if="currentCountry">
                <div class="flag">
      ...
      <div class="btn">
        <button
          :class="['button', visaModule.canUpdate ? '' : 'cantPost']"
          v-html="visaModule.updateButton"
        ></button>
      </div>
    </form>
  </div>
...
currentVisa.value = store.state.visaModule.visa.find(
  (v) => v._id == router.currentRoute.value.params.visa_id
)
countries.value = store.state.visaModule.countries

onMounted(() => {
  if (currentVisa.value) {
    name.value = currentVisa.value.name
    thumbEl.value.src = `${store.state.url}/${currentVisa.value.thumbnail.path}`
    currentCountry.value = countries.value.find((c) => c.flags.png == currentVisa.value.flag)
  }
})
