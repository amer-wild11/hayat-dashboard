<template>
  <div class="updateDest" v-if="currentDest">
    ...
    <form @submit.prevent="upload">
      ...
      <div class="thumbnail">
        ...
        <div class="thumbEl">
          <img :src="store.state.url + '/' + currentDest.thumbnail.path" ref="thumbEl" alt="" />
        </div>
      </div>
      <div class="btn">
        <button
          class="button"
          @click="[(shouldUpload = false), $router.push('/travel&tourism/destinations')]"
        >
          cancel
        </button>
        <button
          v-html="store.state.destsModule.updateButton"
          :class="['button', destsMd.canUpdate ? '' : 'cantPost']"
        ></button>
      </div>
    </form>
  </div>
...
const dests = store.state.destsModule.dests
const destsMd = store.state.destsModule
const currentDest = dests.find((d) => d._id == currentRoute.params.dest_id)
const stars = ref('')
const name = ref('')
const loc = ref('')
const desc = ref('desc')
...
if (currentDest) {
  loc.value = currentDest.location
  name.value = currentDest.name
  desc.value = currentDest.desc
}
