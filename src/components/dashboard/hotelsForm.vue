<template>
  <div class="offersForm" v-if="store.state.hotelsModule.hotelsForm">
    <div class="form">
      <div class="close" @click="store.state.hotelsModule.hotelsForm = false">
        <fa-icon icon="xmark" />
      </div>
      <div class="upload">
      ...
      <div class="content">
        <div class="name-offer">
          ...
          <div class="offer">
            <input type="number" max="100" min="0" v-model="offer" @input="validate" />
            %off
          </div>
        </div>
        <div class="location">
...
const validate = () => {
  ...
  offer.value = Math.floor(offer.value)
}
...
const upload = () => {
  ...
  if (name.value != '' && location.value != '' && thumbInput.value.files.length > 0) {
    ...
    store.commit('postHotel', formData)
  }
}
</script>
