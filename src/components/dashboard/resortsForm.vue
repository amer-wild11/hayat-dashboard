<template>
  <div class="offersForm" v-if="store.state.resortsModule.resortsForm">
    <div class="form">
      <div class="close" @click="store.state.resortsModule.resortsForm = false">
        <fa-icon icon="xmark" />
      </div>
      <div class="upload">
...
const upload = () => {
  ...
  if (name.value != '' && location.value != '' && thumbInput.value.files.length > 0) {
    ...
    store.commit('postResort', formData)
  }
}
</script>
