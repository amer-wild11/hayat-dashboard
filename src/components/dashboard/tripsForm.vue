<template>
  <div class="offersForm" v-if="store.state.tripsModule.tripsForm">
    <div class="form">
      <div class="close" @click="store.state.tripsModule.tripsForm = false">
        <fa-icon icon="xmark" />
      </div>
      <div class="upload">
...
const upload = () => {
  ...
  if (name.value != '' && location.value != '' && thumbInput.value.files.length > 0) {
    ...
    store.commit('postTrip', formData)
  }
}
</script>
