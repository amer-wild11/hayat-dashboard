<template>
  <div class="offersForm" v-if="store.state.destsModule.destsForm">
    <div class="form">
      <div class="close" @click="store.state.destsModule.destsForm = false">
        <fa-icon icon="xmark" />
      </div>
      <div class="upload">
...
const upload = () => {
  ...
  if (name.value != '' && location.value != '' && thumbInput.value.files.length > 0) {
    ...
    store.commit('postDest', formData)
  }
}
</script>
